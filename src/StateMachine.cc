...
ProviderMsgs StateMachine::Transition(const Draw &msg) {
  ...
  // broadcast msg doesn't need to include userId of the player who sent
  // UserOperation. because in client side, it can be deduced from
  // `mCurrentPlayer` in state machine
  ProviderMsgs msgs;
  msgs.push_back(ProviderMsgBuilder::CreateNotifyMsgArgs(
      0, Core::ErrorNumber::OK, roomId, 0, MsgBuilder::CreateDraw(msg)));
  msgs.push_back(ProviderMsgBuilder::CreateNotifyMsgArgs(
...
ls
    ...
    ProviderMsgs
    StateMachine::Transition(const Skip &msg) {
  Logger::Log(msg);
  auto roomId = mState.mCurrentRoomId;
  auto userId = mState.mCurrentUserId;
  ...
  ProviderMsgs msgs;
  msgs.push_back(ProviderMsgBuilder::CreateNotifyMsgArgs(
      0, Core::ErrorNumber::OK, roomId, 0, MsgBuilder::CreateSkip(msg)));
  return msgs;
}
...
ProviderMsgs StateMachine::Transition(const Play &msg) {
  ...
  ProviderMsgs msgs;
  msgs.push_back(ProviderMsgBuilder::CreateNotifyMsgArgs(
      0, Core::ErrorNumber::OK, roomId, 0, MsgBuilder::CreatePlay(msg)));
  return msgs;
}
} // namespace Texas
} // namespace SailGame